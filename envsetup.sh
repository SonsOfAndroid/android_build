...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^soa_") ; then
       SOA_BUILD=$(echo -n $1 | sed -e 's/^soa_//g')
    else
       SOA_BUILD=
    fi
    export SOA_BUILD
...
function brunch()
{
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "No such item in brunch menu. Check your code*/"
        return 1
    fi
    return $?
}
...
{
    target=$1
    SOA_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/soa/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the SOA model name
            lunch soa_$target-userdebug
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        MODVERSION=`sed -n -e'/ro\.soa\.version/s/.*=//p' $OUT/system/build.prop`
        ZIPFILE=soa-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.soa.device=$SOA_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $SOA_BUILD, run away!"
    fi
}
